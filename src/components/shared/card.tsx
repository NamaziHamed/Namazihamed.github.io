import Paragraph from "./paragraph";
import SoftTitle from "./SoftTitle";
import Tag from "./tag";

interface CardProps{
    key:number;
    degree:string;
    school:string;
    period:string;
    description:string;
    subjects:string[];
}


export default function Card ({key,degree,school,period,description,subjects,}:CardProps){
    return (
        <div key={key} className="dark:bg-gray-900 bg-gray-400/10
        p-10 rounded-2xl m-6 md:m-10 lg:m-16 flex flex-col gap-4">
            <SoftTitle>{degree}</SoftTitle>
            <Paragraph className="text-sm ">{period}</Paragraph>
            <Paragraph className="font-semibold">{school}</Paragraph>
            <Paragraph className="text-sm ">{description}</Paragraph>
            <div className="flex gap-2 mx-auto mt-5 flex-wrap">
            {subjects.map((
                (skill,k)=>(
                    <Tag key={k}>{skill}</Tag>
                )
            ))}
            </div>
        </div>
    )
}